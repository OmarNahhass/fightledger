    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '48px', textAlign: 'center' }}>
          <div style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '4px' }}>{view === 'following' ? "You're not following anyone yet" : 'No bets logged yet'}</div>
          <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{view === 'following' ? 'Switch to "Everyone" and follow some bettors' : 'Be the first to place a bet'}</div>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {filtered.map((u, i) => {
            ...
            return (
              <div key={u.userId} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                  ...
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                      ...
                      {isMe && <span style={{ fontSize: '11px', color: 'var(--text-muted)', background: 'var(--bg-hover)', padding: '1px 7px', borderRadius: '4px' }}>You</span>}
                    </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{ fontSize: '11px', color: 'var(--text-muted)' }}>ROI {u.roi >= 0 ? '+' : ''}{u.roi}%</div>
                  </div>
                  {!isMe && (
                    <button
                      ...
                      style={{
                        ...
                        background: isFollowing ? 'var(--bg-hover)' : '#1a1a1a',
